...
| 2d | ~~Copy out Divi's Integration and Custom CSS boxes~~ — **not needed, skipped deliberately.** Nothing in them carries over (CFI's GTM snippet is replaced by the theme; statewide's GTM comes from the GTM4WP plugin, not Divi; the a11y blocks patch Divi markup that will not exist; the custom CSS targets Divi selectors). And it is **not a one-way door**: Divi's Integration code lives in the `et_divi` option in the database, which the cutover preserves along with the old install — so these boxes stay readable indefinitely. Conditional on item 10 | — | ☑ |
| 2i | ~~Does statewide's GTM4WP plugin push dataLayer variables that GTM triggers depend on?~~ — **checked 4 Aug: no pre-populated dataLayer on the page, so nothing is lost when the plugin goes.** This was a real risk worth ruling out: GTM4WP normally publishes post/page variables, and any trigger relying on them would have failed silently at cutover | — | ☑ |
...
- On YouTube itself, add a description to the **Private Flood Insurance VS FEMA** video — Studio
  shows "Add description" and the other three have one. It affects ranking on YouTube, which is a
  separate engine from web search. The body of `/private-flood-insurance-vs-fema/` is the source.
