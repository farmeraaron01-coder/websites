...
| 2d | Copy out **Divi → Theme Options → Integration** (four boxes: head, body, top of posts, bottom of posts) and **→ Custom CSS**, on both sites. Read from the live pages 4 Aug, here is what to expect — CFI: the hand-placed **GTM snippet** (head) and its noscript (body), plus **`claude-a11y-fixes v10`**. Statewide: GTM comes from the **GTM4WP plugin**, not Divi, so its boxes are likely near-empty apart from **`claude-a11y-fixes v6`**. Both carry custom CSS targeting Divi selectors (`#et-top-navigation`, `.container`, `#main-header`) | Aaron | ☐ |
