...
- **statewidefloodinsurance.com** (clone of californiaflood — checked specifically): clean at BOTH levels. 151-URL crawl = 0 spam (incl. its /faqs/); read-only DB SELECT on `mrtaco5_sta1313402424131` (prefix `wp_`) for the hidden-div signature + watchesbuy/tomfordreplica/`.ru/` = 0 rows incl. revisions/unpublished. Did not inherit californiaflood's injection.
- **Second hosting plan (separate cPanel account, ~22 domains incl. jumpins.com):** jumpins.com verified clean on BOTH axes (78-URL content crawl + Wordfence High-Sensitivity file scan = no malware, only 2 plugin-update notices). Remaining plan-2 sites: File Manager plugin-folder inventory pending / covered by InMotion account scan. Note 3 broken installs to address: newmexicofloodinsurance.com, oregonfloodinsurance.com, floridafloodinsurance.net.
